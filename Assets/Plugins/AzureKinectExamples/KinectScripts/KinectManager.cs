...
namespace com.rfilkov.kinect
{
    ...
    public class KinectManager : MonoBehaviour
    {
        ...
        [Tooltip("Whether to create the sensor interfaces according to the multi-camera config, if one is available. Otherwise use the sensor interfaces, as configured in the child objects of this object.是否根据多摄像机配置创建传感器接口（如果有）。否则使用传感器接口，如在该对象的子对象中配置的。")]
        public bool useMultiCamConfig = false;

        [Tooltip("Whether to synchronize the received frames, in case of master/sub cameras.如果是主/副摄像机，是否同步接收到的帧。")]
        public bool syncMultiCamFrames = false;

        [Header("Sensor Data 传感器数据")]

        [Tooltip("Whether to get depth frames from the sensor(s).是否从传感器获取深度帧。")]
        ...
        [Tooltip("Whether to get color frames from the sensor(s).是否从传感器获取彩色帧。")]
        public ColorTextureType getColorFrames = ColorTextureType.None;
        public enum ColorTextureType : int { None = 0, ColorTexture = 2 }

        [Tooltip("Whether to get infrared frames from the sensor(s).是否从传感器获取红外帧。")]
        ...
        [Tooltip("Whether to get pose frames from the sensor(s).是否从传感器获取姿势帧。")]
        ...
        [Tooltip("Whether to get body frames from the body tracker.是否从身体跟踪器获取身体帧。")]
        ...
        [Tooltip("Whether to poll the sensor frames in separate threads or in the Update-method.是在单独的线程中还是在更新方法中轮询传感器帧。")]
        private bool pollFramesInThread = true;

        [Tooltip("Whether to synchronize depth and color frames.是否同步深度和颜色帧。")]
        public bool syncDepthAndColor = false;

        [Tooltip("Whether to synchronize body and depth frames.是否同步主体帧和深度帧。")]
        ...
        [Header("User Detection 用户检测")]

        [Tooltip("Minimum distance to user, in order to be considered for body processing. Value of 0 means no minimum distance limitation.与用户的最小距离，以便考虑车身加工。值为0表示没有最小距离限制。")]
        [Range(0f, 10f)]
        public float minUserDistance = 0f;

        [Tooltip("Maximum distance to user, in order to be considered for body processing. Value of 0 means no maximum distance limitation.与用户的最大距离，以便考虑车身处理。值为0表示没有最大距离限制。")]
        [Range(0f, 10f)]
        public float maxUserDistance = 0f;

        [Tooltip("Maximum left or right distance to user, in order to be considered for body processing. Value of 0 means no left/right distance limitation.与用户的最大左或右距离，以便考虑车身处理。值为0表示没有左/右距离限制。")]
        [Range(0f, 5f)]
        public float maxLeftRightDistance = 0f;

        [Tooltip("Maximum number of users, who may be tracked simultaneously. Value of 0 means no limitation.可同时跟踪的最大用户数。值为0表示没有限制。")]
        public int maxTrackedUsers = 0;

        [Tooltip("Whether to display only the users within the allowed distances, or all users.是仅显示允许距离内的用户，还是显示所有用户。")]
        public bool showAllowedUsersOnly = false;

        public enum UserDetectionOrder : int { Appearance = 0, Distance = 1, LeftToRight = 2 }
        [Tooltip("How to assign users to player indices - by order of appearance, distance or left-to-right.如何分配用户的球员指数-按顺序的外观，距离或从左到右。")]
        public UserDetectionOrder userDetectionOrder = UserDetectionOrder.Appearance;

        [Tooltip("Whether to ignore the inferred joints, or consider them as tracked joints.是否忽略推断的关节，或认为它们是跟踪关节。")]
        public bool ignoreInferredJoints = false;

        [Tooltip("Whether to ignore the Z-coordinates of the joints (for 2D-scenes) or not.是否忽略关节的Z坐标（对于二维场景）。")]
        ...
        [Tooltip("Wait time in seconds, before a lost user gets removed. This is to prevent sporadical user switches.以秒为单位的等待时间，在丢失的用户被删除之前。这是为了防止偶尔的用户切换。")]
        protected float waitTimeBeforeRemove = 0f;

        [Tooltip("Calibration pose required, to start tracking the respective user.需要校准姿势，才能开始跟踪相应的用户。")]
        public GestureType playerCalibrationPose = GestureType.None;

        [Tooltip("User manager, used to track the users. KM creates one, if not set.用户管理器，用于跟踪用户。如果未设置，KM将创建一个。")]
        ...
        [Tooltip("Gesture manager, used to detect user gestures. KM creates one, if not set.手势管理器，用于检测用户手势。如果未设置，KM将创建一个。")]
        ...
        [Header("On-Screen Info 屏幕信息")]

        [Tooltip("List of images to display on the screen. 要在屏幕上显示的图像列表。")]
        ...
        [Tooltip("Single image width, as percent of the screen width. The height is estimated according to the image's aspect ratio.单个图像宽度，占屏幕宽度的百分比。根据图像的纵横比估计高度。")]
        [Range(0.1f, 0.5f)]
        public float displayImageWidthPercent = 0.2f;

        [Tooltip("UI-Text to display status messages.显示状态消息的UI文本。")]
        public UnityEngine.UI.Text statusInfoText;

        [Tooltip("Whether to log the KinectManager info messages to the console or not.是否将KineTManager信息消息记录到控制台。")]
