...
namespace com.rfilkov.kinect
{
    ...
    public abstract class DepthSensorBase : MonoBehaviour, DepthSensorInterface
    {
        ...
        [Tooltip("Device streaming mode, in means of connected sensor, recording or disabled.设备流模式，通过连接的传感器，记录或禁用。")]
        public KinectInterop.DeviceStreamingMode deviceStreamingMode = KinectInterop.DeviceStreamingMode.ConnectedSensor;

        [Tooltip("Index of the depth sensor in the list of currently connected sensors.当前连接的传感器列表中深度传感器的索引。")]
        public int deviceIndex = 0;

        [Tooltip("Path to the recording file, if the streaming mode is PlayRecording.如果流模式为播放录制，则指向录制文件的路径。")]
        ...
        [Tooltip("Minimum distance in meters, used for creating the depth-related images.最小距离（米），用于创建与深度相关的图像。")]
        [Range(0f, 10f)]
        public float minDistance = 0.5f;

        [Tooltip("Maximum distance in meters, used for creating the depth-related images.最大距离（米），用于创建与深度相关的图像。")]
        [Range(0f, 10f)]
        public float maxDistance = 10f;

        [Tooltip("Resolution of the generated point-cloud textures.生成的点云纹理的分辨率。")]
        ...
        [Tooltip("Render texture, used for point-cloud vertex mapping. The texture resolution should match the depth or color image resolution.渲染纹理，用于点云顶点映射。纹理分辨率应与深度或彩色图像分辨率匹配。")]
        public RenderTexture pointCloudVertexTexture = null;

        [Tooltip("Render texture, used for point-cloud color mapping. The texture resolution should match the depth or color image resolution.渲染纹理，用于点云颜色映射。纹理分辨率应与深度或彩色图像分辨率匹配。")]
        public RenderTexture pointCloudColorTexture = null;

        [Tooltip("List of comma-separated player indices to be included in the point cloud. Use -1 for all players, or empty list for full point cloud.要包含在点云中的以逗号分隔的播放器索引的列表。所有玩家使用-1，满分云使用空列表。")]
