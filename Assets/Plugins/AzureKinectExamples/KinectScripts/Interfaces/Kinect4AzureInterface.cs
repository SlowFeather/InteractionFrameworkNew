...
namespace com.rfilkov.kinect
{
    ...
    {
        [Tooltip("Color camera resolution.彩色摄像机分辨率。")]
        ...
        [Tooltip("Depth camera mode.深度相机模式。")]
        ...
        [Tooltip("Camera frames per second.每秒相机帧数。")]
        public CameraFPS cameraFPS = CameraFPS._30_FPS;
        public enum CameraFPS : int { _5_FPS = 0, _15_FPS = 1, _30_FPS = 2 }

        [Tooltip("Device sync mode, in case of multiple wired sensors.设备同步模式，在多个有线传感器的情况下。")]
        public WiredSyncMode deviceSyncMode = WiredSyncMode.Standalone;

        [Tooltip("Subordinate device delay off master (in usec), in case of multiple wired sensors.在多个有线传感器的情况下，从设备延迟关闭主设备（在usec中）。")]
        public int subDeviceDelayUsec = 0;

        [Tooltip("Whether to flip the IMU rotation.是否翻转IMU旋转。")]
        private bool flipImuRotation = false;

        [Tooltip("Temporal smoothing used by the Azure Kinect Body Tracking SDK.Azure Kinect身体跟踪SDK使用的时间平滑。")]
        [Range(0f, 1f)]
        public float bodyTemporalSmoothing = 0f;

        [Tooltip("Playback position in seconds.播放位置（秒）。")]
        [Range(-1f, 30f)]
        public float playbackPosSeconds = -1f;

        [Tooltip("Whether to loop the playback, when end of recording is reached.当到达录制结束时，是否循环播放。")]
        public bool loopPlayback = false;

        [Tooltip("Sensor orientation hint to the Body Tracking SDK.身体跟踪SDK的传感器方向提示。")]
        ...
        [Tooltip("Whether to try to detect the floor plane, to estimate the sensor position and rotation.是否尝试检测地板平面，估计传感器的位置和旋转。")]
        ...
        private bool InitBodyTracking(KinectInterop.FrameSource dwFlags, KinectInterop.SensorData sensorData, Calibration calibration, bool bCreateTracker)
        {
            try
            {
                if ((dwFlags & KinectInterop.FrameSource.TypeDepth) != 0)  // check for depth stream
                {
                    ...
                    if (bCreateTracker)
                    {
                        //bodyTracker = new BodyTracking(calibration, bodyTrackingSensorOrientation, k4abt_tracker_processing_mode_t.K4ABT_TRACKER_PROCESSING_MODE_CPU, 0);
